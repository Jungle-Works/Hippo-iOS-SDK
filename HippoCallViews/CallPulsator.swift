...
internal let kCallPulsatorAnimationKey = "pulsator"

open class CallPulsator: CAReplicatorLayer, CAAnimationDelegate {
    ...
    @objc internal func resume() {
        ...
        let isAnimating = pulse.animation(forKey: kCallPulsatorAnimationKey) != nil
        // if the animationGroup is not nil, it means the animation was not stopped
        if let animationGroup = animationGroup, !isAnimating {
            pulse.add(animationGroup, forKey: kCallPulsatorAnimationKey)
        }
    }
