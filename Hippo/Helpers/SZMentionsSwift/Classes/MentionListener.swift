...
public class MentionListener: NSObject {
    ...
    public func updateDefaultTextAttributes(newAttributes: [AttributeContainer]) {
        self.defaultTextAttributes = newAttributes
    }
}
...
//                let endIndex = mentionsTextView.text.index(startIndex, offsetBy: NSMaxRange(textView.selectedRange) - location)
                let length = NSMaxRange(textView.selectedRange) - location
                
                if length > 0 {
                    let range = NSRange(location: location, length: length)
                    mentionString = (mentionsTextView.text as NSString).substring(with: range)
                } //String(mentionsTextView.text[startIndex ..< endIndex])
