...
class SocketClient: NSObject {
    ...
    private func socketSetup(){
        ...
        if let url = URL(string: socketURL){
            manager = SocketManager(socketURL: url, config: [.reconnectWait(Int(2)), .reconnectAttempts(20), .compress, .forcePolling(false), .forceWebsockets(true), .connectParams(["auth_token" : encryptedAuth, "device_type" : Device_Type_iOS, "device_details" : AgentDetail.getDeviceDetails()])])
        }
