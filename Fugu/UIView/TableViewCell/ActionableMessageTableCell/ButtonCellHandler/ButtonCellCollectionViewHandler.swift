...
class ButtonCellCollectionViewHandler: NSObject {
    ...
    var chatMessageObj: FuguMessage?
...
//            }
    ...
    @objc func messageButtonClicked(sender: UIButton) {
        ...
        if let buttonActionArray = chatMessageObj?.actionableMessage?.actionButtonsArray {
            ...
            if  buttonActionArray.count > sender.tag, let buttonInfo =  buttonActionArray[sender.tag] as? [String: Any] {
                HippoConfig.shared.delegate?.fuguMessageRecievedWith(response: buttonInfo, viewController: self.rootViewController!)
                }
            }
