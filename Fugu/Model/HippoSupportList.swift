...
struct UserDefaultsKey {
    static let TicketsKey = "HippoTicketKey"
    static let currentFAQVersion = "currentFAQVersion"
}
...
class HippoSupportList: NSObject {
    ...
    static var currentFAQVersion: Int {
        get {
            return UserDefaults.standard.value(forKey: UserDefaultsKey.currentFAQVersion) as? Int ?? 0
        }
        set {
            UserDefaults.standard.set(newValue, forKey: UserDefaultsKey.currentFAQVersion)
        }
    }
    static var FAQData: [String: Any] {
        get {
            return UserDefaults.standard.value(forKey: UserDefaultsKey.TicketsKey) as? [String: Any] ?? [:]
        }
        set {
            UserDefaults.standard.set(newValue, forKey: UserDefaultsKey.TicketsKey)
        }
    }
    ...
    var content = FuguDescriptionContent()
    ...
    init(json: [String: Any]) {
        ...
        if let content = json["content"] as? [String: Any] {
            self.content = FuguDescriptionContent(json: content)
        }
    ...
    class func getListForBusiness(completion: ((_ success: Bool, _ objectArray: [HippoSupportList]?) -> Void)? = nil) {
        ...
        HTTPClient.makeConcurrentConnectionWith(method: .POST, para: params,  extendedUrl: FuguEndPoints.API_GET_SUPPORT_DATA.rawValue) {(responseObject, error, tag, statusCode) in
            switch (statusCode ?? -1) {
