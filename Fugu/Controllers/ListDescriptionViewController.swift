...
class ListDescriptionViewController: UIViewController {
    ...
    let min_height_textview: CGFloat = 120
...
//        let tags = [HippoSupportList.FAQName]
        ...
        var params = FuguChannel.getParamsToStartConversation(fuguAttributes: attributes)
        params["tags"] = [["tag_name": HippoSupportList.FAQName]]
        
        
        FuguChannel.createNewConversationForTicket(params: params, completion: {(result) in
            self.submitButton.stopLoading()
    ...
    func presentChatScreen(with message: String) {
        ...
        HippoConfig.shared.checkForIntialization { (success, error) in
            ...
            var fuguChat = FuguNewChatAttributes(transactionId: transactionId, userUniqueKey: id, otherUniqueKey: nil, tags: tags, channelName: channelName, preMessage: message)
            fuguChat.customAttributes = self.getCustomAttributes()
            fuguChat.isCustomAttributesRequired = true
            fuguChat.isInAppChat = true
...
//        updateHeightOf(textView: textView)
    ...
    func updateHeightOf(textView: UITextView) {
        ...
        DispatchQueue.main.async {
            self.textViewContainerHeightConstraint.constant = self.max_height_textview + 25
