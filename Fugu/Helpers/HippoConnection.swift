...
public enum HippoConnectionError: Error {
...
extension Notification.Name {
    public static let HippoConnectionChanged = Notification.Name("HippoConnectionChanged")
}
...
func callback(reachability:SCNetworkReachability, flags: SCNetworkReachabilityFlags, info: UnsafeMutableRawPointer?) {
    ...
    let reachability = Unmanaged<HippoConnection>.fromOpaque(info).takeUnretainedValue()
    reachability.reachabilityChanged()
}

public class HippoConnection {
    
    public typealias NetworkReachable = (HippoConnection) -> ()
    public typealias NetworkUnreachable = (HippoConnection) -> ()
...
public extension HippoConnection {
    ...
    func startNotifier() throws {
        ...
        context.info = UnsafeMutableRawPointer(Unmanaged<HippoConnection>.passUnretained(self).toOpaque())
        if !SCNetworkReachabilitySetCallback(reachabilityRef, callback, &context) {
            stopNotifier()
            throw HippoConnectionError.UnableToSetCallback
        }
        
        if !SCNetworkReachabilitySetDispatchQueue(reachabilityRef, reachabilitySerialQueue) {
            stopNotifier()
            throw HippoConnectionError.UnableToSetDispatchQueue
        }
...
fileprivate extension HippoConnection {
    ...
    func reachabilityChanged() {
        ...
        DispatchQueue.main.async {
            block?(self)
            self.notificationCenter.post(name: .HippoConnectionChanged, object:self)
        }
        previousFlags = flags
    }
