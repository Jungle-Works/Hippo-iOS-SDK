...
class FuguNetworkHandler: NSObject {
   ...
   static let shared = FuguNetworkHandler()
    ...
    let reachability = HippoConnection()!
    ...
    fileprivate func updateConnectedStatus() {
            ...
            self.errorMessage(errorLabelColor: UIColor.green, errorLabelMessage: "Connected", isToLoadFirstTime: false, needToBeHidden: false)
            fuguDelay(0.5, completion: {
                self.errorMessage(errorLabelColor: UIColor.green, errorLabelMessage: "Connected",  needToBeHidden: true)
            })
    }
    ...
    fileprivate func errorMessage(errorLabelColor: UIColor? = nil, errorLabelMessage: String = "", isToLoadFirstTime: Bool = true, needToBeHidden hidden: Bool) {
        let erorMessage = errorLabelMessage.isEmpty ? "No Internet Connection" : errorLabelMessage
